# ==============================================================================
# CWSRF Cash Flow Model
# Author: Katy Hansen
# Date: 9/7/25
# Description: This script models the cash flow of State Revolving Funds (SRFs)
...
# Discover and load raw data files for each state.
# Source Note: Data from EPA Annual Summary Reports.
# ==============================================================================
cat("Loading all Excel files...\n")

# --- Define expected filenames based on state codes ---
expected_files <- list.files(path = "data-updates/raw-data/cw_state_data", full.names = FALSE)

state_codes_from_names <- str_extract(expected_files, "^[A-Z]{2}")

# --- Set data directory path ---
data_directory <- "data-updates/raw-data/cw_state_data"
...
#' 1. Earmark data from a prepared CSV file.
...
load_state_data <- function(state_abbr) {
  ...
  earmarks_data <- read.csv("data-updates/clean-data/srf_funding_data_update_fy26.csv") %>%
    mutate(state_abbr = if_else(is.na(state_abbr),
                                ...
                                state_abbr)) %>%
    filter(program == "CWSRF", state_abbr == !!state_abbr) %>%
    transmute(
  ...
  line_mapping <- c(`70` = "cap_grant", `104` = "executed_loans",
                    `324` = "subsidy", `240` = "loan_rate", `236` = "reported_repayments")
  
  # Data wrangling process:
  # 1. Filter raw data to include only relevant lines based on line_mapping.
  # 2. Pivot data from wide format (one column per year) to long format.
  # 3. Clean values (remove non-numeric characters) and assign variable names.
  # 4. Pivot data back to wide format (one column per variable).
...
# --- Create Plot Object ---
cw_waterfall_plot <- ggplot(waterfall, aes(y = state)) +
  ...
  # MODIFIED: Colors reverted to match the previous plot
  ...
  # MODIFIED: Font size reverted to 9 to match the previous plot
  theme_minimal(base_size = 9, base_family = "Lato") +
  theme(
    # All text elements are now explicitly set to size 9 for consistency.
...
dir.create("results-updates/section4/CW/state_repayment_graphs", recursive = TRUE, showWarnings = FALSE)
dir.create("results-updates/section4/CW/state_repayment_data", recursive = TRUE, showWarnings = FALSE)
...
for (state_abbr in all_states) {
  ...
  write.csv(
    ...
    file = paste0("results-updates/section4/CW/state_repayment_data/",
